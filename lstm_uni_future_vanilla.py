...
def create_vanilla_lstm(X, y, timestep, epochs, batchsize, neuron):
...
for i in range(1, 16):
    timestep = i
    times.append(i)
    filename = f'V_LSTM_timestep_{i}'
    plotname = f'Timesteps: {i}'
    X_train, X_test, y_train, y_test = create_x_y_datasets(scaled_input, timestep, test_day_number)
    model, history = create_vanilla_lstm(X_train, y_train, timestep, epochs, batch_size, neuron)
...
imageio.mimsave(f'{filepath}_V_LSTM_uni_future.gif', images_future, fps=2)
imageio.mimsave(f'{filepath}_V_LSTM_uni_test.gif', images_test, fps=2)
...
plt.ylabel('RMSE')
plt.savefig(f'{filepath}/RMSE_vanilla.jpg')
